...
public class VirtualButtonEventHandler : MonoBehaviour, IVirtualButtonEventHandler {
	// IVirtualButtonEventHandler insure OnButtonPressed and OnButtonReleased method are implemented 
	...
	void Start() {
		...
		for (int i = 0; i < vbs.Length; ++i) {
			...
			// Register button handler to vuforia event handler
			// Register the button as active event
			vbs[i].RegisterEventHandler(this);
		}

		// Find the models based on the names in the Hierarchy
		model_1 = transform.FindChild("WarGreymon").gameObject;
